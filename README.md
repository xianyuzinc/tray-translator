...
- Editable source text: fix or rewrite the captured text in the popup, then click `Retranslate` or press `Ctrl+Enter`.
...
6. Edit the source text in the top input box when needed, then click `重译` or press `Ctrl+Enter`.
7. Keep the popup open and select another text range in another app to translate again automatically.
8. For long text, resize the popup from its right/bottom edge or click `展开` on a result card to open the reader window.
...
- 原文可编辑：可在浮窗顶部修正或改写识别到的文本，再点 `重译` 或按 `Ctrl+Enter`。
...
6. 如需修正原文，可直接编辑浮窗顶部输入框，然后点 `重译` 或按 `Ctrl+Enter`。
7. 保持浮窗打开，切换到其他应用重新选中文字，即可自动重新翻译。
8. 遇到长文本时，可以拖动浮窗右边缘/下边缘放大，也可以点击结果卡片里的 `展开` 打开阅读窗。
